			...
			<c:if test="${pageHandler.listCount > 0}">
						...
						<div class="col">
							<div class="card w-100">
								...
								<div class="card-body text-center">
									...
									<a href="/product/detail?product_code=${newProduct.product_code}" class="product-list-title">
										<h4 class="card-title mb-3">${newProduct.product_name}</h4>
									</a>
									<p class="card-text fs-4">
										<fmt:formatNumber var="newProductPrice" value="${newProduct.product_price}" pattern="###,###,###" />
										<span>가격 : ${newProductPrice}원</span>
									</p>
									<a href="/product/detail?product_code=${newProduct.product_code}" class="btn btn-primary w-100">상세보기</a>
								</div>
							</div>
						</div>
					</c:forEach>
				...
				<nav class="d-flex justify-content-center">
					<ul class="pagination">
						...
						<c:if test="${pageHandler.page > 1}">
							<li class="page-item"><a class="page-link" href="/product/newProductList?page=${pageHandler.page-1}" aria-label="Previous"> <span aria-hidden="true">&laquo;</span>
							</a></li>
						</c:if>
						<c:forEach var="a" begin="${pageHandler.startPage}" end="${pageHandler.endPage}">
							...
							<c:if test="${a != pageHandler.page}">
								<li class="page-item"><a class="page-link" href="/product/newProductList?page=${a}">${a}</a></li>
							</c:if>
						</c:forEach>
						...
						<c:if test="${pageHandler.page < pageHandler.maxPage}">
							<li class="page-item"><a class="page-link" href="/product/newProductList?page=${pageHandler.page+1}" aria-label="Next"><span aria-hidden="true">&raquo;</span></a></li>
						</c:if>
					</ul>
				</nav>
			</c:if>
